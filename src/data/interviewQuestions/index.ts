// Interview Question Bank Engine & Registry
...
export function selectInterviewQuestions(options: SelectQuestionsOptions): InterviewQuestion[] {
  ...
  const highDemandSkills = new Set(
    industrySkills.filter(s => s.demandScore > 75 || s.importance === 'Critical').map(s => s.name.toLowerCase())
  );

  // Student skill gaps (unverified skills or beginner level)
  const skillGapSet = new Set(
    studentSkills.filter(s => !s.verified || s.level === 'Beginner').map(s => s.name.toLowerCase())
  );
